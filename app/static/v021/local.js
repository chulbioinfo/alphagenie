...
async function request(url,body){
  const response=await fetch(url,{credentials:'same-origin',cache:'no-store',...(body===undefined?{}:{method:'POST',headers:{'Content-Type':'application/json','X-AlphaGENIE-Token':token},body:JSON.stringify(body)})});
...
export async function enterLocal(){
  active=true;
  if(!initialized){
    // No account or browser login: the local HTTP listener is not an OS-user
    // isolation boundary. File permissions do not authorize HTTP requests.
...
python -m alphagenie doctor</pre><p>The key is entered privately in the terminal, not this browser. It is stored in an owner-only local file outside the repository (not encrypted). Environment-only credentials are also supported. See docs/PRIVACY.md and docs/INSTALL.md.</p></details></section>
    ...
    initialized=true;
    $('#localConfig').insertAdjacentHTML('beforebegin','<p class="notice">Personal computer only. No login or user isolation; do not run on shared computers or expose this server.</p>');
