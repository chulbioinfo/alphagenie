...
function dialog(name){
  ...
  if(name==='about'){openDialog('About AlphaGENIE','<p>AlphaGENome-Integrated Explorer</p><p><strong>Interface v0.24 · data v0.24</strong></p><p>Explore saved AlphaGenome-derived RNA predictions and export publication figures. Viewing results and changing figure layouts do not submit new AlphaGenome jobs. This local edition supports fresh analyses with your own API key through My analyses. The key is managed locally and is sent to Google DeepMind only for API authentication; AlphaGENIE operators do not collect it.</p><h3>Personal-computer use only</h3><p>No account, login or password recovery is required. Use a private computer and a single trusted OS account. Other programs or users on the same computer may access the local service and job outputs; browser request checks are not local-user authentication. Do not use a shared server, tunnel or public-facing proxy. Keep your browser, OS and local programs trusted and up to date.</p><h3>Attribution & use</h3><p>AlphaGenome is developed by Google DeepMind. AlphaGENIE is an independent research interface, not an official Google product and not endorsed by Google DeepMind.</p>'+usageNotice()+'<h3>Modifications</h3><p>AlphaGENIE applies matched-null calibration, tissue grouping, statistical summaries and visualization to AlphaGenome-derived outputs. v0.24 includes reviewed inference guards and cross-validation tools. The saved artifacts retain their v0.20 source lineage; a fresh fixed-seed provider run reproduced their values, with separate web and GitHub aggregation.</p><p>Scores are model predictions, not measurements, clinical diagnoses or evidence of pathogenicity.</p>');return;}
  if(name==='guide'){openDialog('How to read results','<h3>1. Choose a saved result</h3><p>RBFOX1 uses a 16 kb input; PTCHD1 uses a 1 Mb input. Compare variants opens the separate 17-variant, 1 Mb cohort. Switching results does not run a new analysis.</p><h3>2. Inspect the two views</h3><p>The upper view shows the gene and frontal-cortex prediction. The lower view compares adjusted scores across tissue categories. Hover a point, or select it to keep the source details open. Keyboard users can focus a point and press Enter or Space.</p><h3>3. Review evidence, then export</h3><p>Single-variant results show unadjusted empirical P; cohort results use BH q across 17 variants. Color indicates effect direction, not statistical significance or pathogenicity.</p><p>Export figure combines both single-variant views vertically using your dimensions. The approved cohort PDF retains its fixed manuscript layout.</p><details><summary>Other tools</summary><p>Track library lets you search every source track. My analyses provides local reference checks and explicit fresh API submission with your own key. New runs now use the same manuscript Brain9 classification, with non-brain tissue and cell comparison pools. Their RNA curve selects the same adult GTEx Frontal cortex track as saved results, but fresh API values are not the saved inference.</p><button class="text-button" data-dialog="feedback">Feedback →</button></details>');return;}
...
function route(){
  ...
  document.title=state.route==='overview'?'AlphaGENIE · AlphaGENome-Integrated Explorer':$('#pageTitle').textContent+' · AlphaGENIE v0.24';
