...
function dialog(name){
  ...
  if(name==='about'){openDialog('About AlphaGENIE','<p>AlphaGENome-Integrated Explorer</p><p><strong>Interface v0.23 · source data v0.20</strong></p><p>Explore saved AlphaGenome-derived RNA predictions and export publication figures. Viewing results and changing figure layouts do not submit new AlphaGenome jobs. This local edition supports fresh analyses with your own API key through My analyses. The key is managed locally and is sent to Google DeepMind only for API authentication; AlphaGENIE operators do not collect it.</p><h3>Personal-computer use only</h3><p>No account, login or password recovery is required. Use a private computer and a single trusted OS account. Other programs or users on the same computer may access the local service and job outputs; browser request checks are not local-user authentication. Do not use a shared server, tunnel or public-facing proxy. Keep your browser, OS and local programs trusted and up to date.</p><h3>Attribution & use</h3><p>AlphaGenome is developed by Google DeepMind. AlphaGENIE is an independent research interface, not an official Google product and not endorsed by Google DeepMind.</p>'+usageNotice()+'<h3>Modifications</h3><p>AlphaGENIE applies matched-null calibration, tissue grouping, statistical summaries and visualization to AlphaGenome-derived outputs. v0.23 corrects matched-null ID handling and result downloads; frozen v0.20 predictions are unchanged.</p><p>Scores are model predictions, not measurements, clinical diagnoses or evidence of pathogenicity.</p>');return;}
...
function route(){
  ...
  document.title=state.route==='overview'?'AlphaGENIE · AlphaGENome-Integrated Explorer':$('#pageTitle').textContent+' · AlphaGENIE v0.23';
